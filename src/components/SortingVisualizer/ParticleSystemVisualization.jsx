...
const ParticleSystemVisualization = ({ array, currentIndices, getColor }) => {
    ...
    useEffect(() => {
        ...
        const maxDimension = Math.max(gridSize * 0.6, 4); // Adjust this factor to control zoom
        ...
        const particleMaterial = new THREE.ShaderMaterial({
            ...
            fragmentShader: `
                ...
                void main() {
                    ...
                    // Subtle glow effect that preserves original colors
                    vec3 glow = vColor * (1.0 - radius * 1.2);
                    gl_FragColor = vec4(vColor + glow * 0.2, alpha);
                }
            `,
            transparent: true,
        ...
        // Calculate grid positions with tighter spacing
        const spacing = 0.5; // Standard spacing between particles
        const positions = [];
        const colors = [];
        const sizes = [];
        const indices = [];
        ...
        array.forEach((value, idx) => {
            ...
            const color = new THREE.Color(getColor(value, idx));
            // Enhance color saturation
            color.multiplyScalar(1.2);
            colors.push(color.r, color.g, color.b);

            // Smaller particle size
            // Smaller, more defined base size
        ...
        const animate = () => {
            ...
            time += 0.01;
            rotationRef.current += 0.002; // Slower rotation
    ...
    useEffect(() => {
        ...
        array.forEach((value, idx) => {
            ...
            // Update sizes with larger base size
            sizes[idx] = 0.3 + (value / array.length) * 0.2;
        });
